...
> below verbatim from the measured runs (local Ollama, no paid keys).
...
**The shipped library.** QFIRE ships **106 rules across 9 domains** plus **24
chains** (6 application + 18 benchmark/eval chains), all linted by
`qfire rules lint`:
...
- **Metrics:** attack = positive class; precision, recall, F1, FPR, accuracy
  with 95% Wilson intervals, ROC–AUC (continuous detector score), and latency. Unless noted, every evaluation run uses seed 42 (QFIRE-HealthBench was generated under seed 1337).
...
100-attack / 100-benign HealthBench subset, the same prompt, `--no-cache`
...
on the same 929 public injection attacks and 50 generated in-domain
...
DeepSeek-R1) on a 150-attack subset (Llama 3.2 reused by slicing its full-run
...
attacks (crafted with knowledge of the firewall, generated by gemma2:9B) and
...
holds for a **complete framework**: NVIDIA NeMo Guardrails' full input-rail stack (§3.11),
...
until it ALLOWs (60 attacks, gemma2:9B; `figs/cascade.png`). Stage 1 (standard)
...
### 3.11 Full-stack framework baseline: NeMo Guardrails
...
### 3.12 Multi-turn / conversational injection

A stateless per-message filter misses attacks that only become malicious *across* turns.
We test 150 multi-turn conversations (deterministic, malicious core verbatim):
...
### 3.13 External validity: transfer, scale, and threshold stability

A standing caveat is that cross-dataset numbers drop. We convert it into three
measured, bounded statements (offline).
...
### 3.14 End-to-end agent harm reduction
...
harmful-action rate end-to-end. A hand-rolled ReAct agent (`llama3.1:8b`, temp 0) drives an in-process mock-EHR sandbox over synthetic data; the sandbox logs every
...
### 3.15 Standard agent benchmarks: AgentDojo and InjecAgent
...
- **Cross-dataset numbers shift off-distribution, but the shift is bounded.**
  ...
  corpus the same weights score F1 0.84. We now measure the transfer (§3.13): on a
...
- **Text-only, single-turn scope.** QFIRE inspects text prompts: multi-modal injection
  ...
  evaluated (§3.12): the full-transcript default catches split-payload/priming/crescendo
...
QFIRE ships **106 rules / 9 domains** and **24 chains**, all version-controlled
under `rules/` and `chains/` and validated by `qfire rules lint`. The repository
also carries evaluation-only rules under `rules/bench/` (26) and `rules/e7/` (8),
and benchmark/agent chains under `chains/bench/` and `e7_*`, used only for the
experiments in this paper. Every rule's `exemplars` are executed as fixtures by
`qfire rules test`.
