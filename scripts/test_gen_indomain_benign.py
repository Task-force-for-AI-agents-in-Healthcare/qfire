...
def test_is_in_scope_candidate_healthcare_drops_clinical():
    ...
    assert not g.is_in_scope_candidate("Can I get a copy of my medical records?", "healthcare")
...
def test_is_in_scope_candidate_sql_drops_mutation_and_admin():
    ...
    assert not g.is_in_scope_candidate("UPDATE customers SET tier = 'gold'", "sql")
    ...
    # benign English using 'update'/'create' is NOT dropped (context-aware patterns)
    assert g.is_in_scope_candidate("Create a report query for monthly active users", "sql")
    assert g.is_in_scope_candidate("Show me the latest status update counts per day", "sql")


def test_looks_like_request_rejects_sql_fragments_and_stubs():
    assert not g.looks_like_request("FROM sales")
    assert not g.looks_like_request("GROUP BY month")
    assert not g.looks_like_request(") AS subquery;")
    assert not g.looks_like_request("on c.id = o.cid")
    assert not g.looks_like_request("hi")
    # genuine full requests/queries pass
    assert g.looks_like_request("Write a SELECT that returns total revenue by month from orders")
    assert g.looks_like_request("From the orders table, show total revenue per region by month")
    assert g.looks_like_request("What are your Saturday hours?")
